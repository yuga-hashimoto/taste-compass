const { LOCAL_IMAGES } = require('./../src/data/imageMetadata.ts');
...
LOCAL_IMAGES.forEach((img) => {
  ...
  if (img.body_silhouette) bodySilhouettes[img.body_silhouette] = (bodySilhouettes[img.body_silhouette] || 0) + 1;
});

console.log('\n--- Style Groups ---');
Object.entries(styles).sort((a,b)=>b[1]-a[1]).forEach(([k, v]) => {
  console.log(`${k.padEnd(20)}: ${v} (${((v/total)*100).toFixed(1)}%)`);
});

console.log('\n--- Regional Styles ---');
Object.entries(regions).sort((a,b)=>b[1]-a[1]).forEach(([k, v]) => {
  console.log(`${k.padEnd(25)}: ${v} (${((v/total)*100).toFixed(1)}%)`);
});

console.log('\n--- Age Impressions ---');
Object.entries(ages).sort((a,b)=>b[1]-a[1]).forEach(([k, v]) => {
  console.log(`${k.padEnd(20)}: ${v} (${((v/total)*100).toFixed(1)}%)`);
});

console.log('\n--- Vibe Types ---');
Object.entries(vibes).sort((a,b)=>b[1]-a[1]).forEach(([k, v]) => {
  console.log(`${k.padEnd(20)}: ${v} (${((v/total)*100).toFixed(1)}%)`);
});

console.log('\n--- Body Silhouettes ---');
Object.entries(bodySilhouettes).sort((a,b)=>b[1]-a[1]).forEach(([k, v]) => {
  console.log(`${k.padEnd(20)}: ${v} (${((v/total)*100).toFixed(1)}%)`);
});
