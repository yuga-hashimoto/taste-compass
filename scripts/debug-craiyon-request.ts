...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector('textarea#prompt, input#prompt, textarea[placeholder*="prompt" i]', { timeout: 10000 });
    const promptInput = await page.$('textarea#prompt, input#prompt, textarea[placeholder*="prompt" i]');
    if (promptInput) {
      ...
      const drawButton = await page.$('button:has-text("Draw"), button#generate, button[type="submit"]');
      if (drawButton) {
